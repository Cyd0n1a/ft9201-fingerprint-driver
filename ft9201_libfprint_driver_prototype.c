 ...
 * The kernel driver uses usb_find_common_endpoints() to locate this
 * dynamically. Verify with `lsusb -v -d 2808:93a9` and adjust if needed. */
#define FT9201_EP_BULK_IN                  0x81
...
struct _FpiDeviceFt9201
{
  ...
  /* GSource ID for the single pending capture retry timer (MCU poll or armed
   * check). Only one timer is ever live at a time. Stored so ft9201_deactivate
   * can cancel it before calling deactivate_complete. 0 = no timer pending. */
...
retry_capture_mcu_poll (gpointer user_data)
{
  ...
  FpiDeviceFt9201 *self = FPI_DEVICE_FT9201 (fpi_ssm_get_device (ssm));

  self->capture_retry_source = 0;
...
retry_capture_armed_poll (gpointer user_data)
{
  ...
  self->capture_retry_source = 0;
  fpi_ssm_jump_to_state (ssm, CAPTURE_CHECK_ARMED);
  return G_SOURCE_REMOVE;
}
                     ...
                     GError         *error)
{
  ...
  if (mcu_is_ready (self->reg_buf))
    ...
    {
      self->capture_retry_source =
        g_timeout_add (FT9201_CAPTURE_MCU_POLL_DELAY_MS,
                       retry_capture_mcu_poll, transfer->ssm);
    }
}
  ...
  else
    {
      ...
      self->capture_retry_source =
        g_timeout_add (FT9201_CAPTURE_ARMED_RETRY_MS,
                       retry_capture_armed_poll, transfer->ssm);
    }
}
...
capture_ssm_state (FpiSsm *ssm, FpDevice *device)
{
  ...
  switch ((CaptureState) fpi_ssm_get_cur_state (ssm))
    ...
    case CAPTURE_START_DELAY:
       ...
       * msleep(20) → fpi_ssm_next_state_delayed(ssm, 20)
       */
      fpi_ssm_next_state_delayed (ssm, FT9201_CAPTURE_CMD_GAP_MS);
      break;
    ...
    case CAPTURE_READ_IMAGE:
      {
        ...
        fp_dbg ("capture: bulk IN — requesting %u bytes", img_sz + FT9201_BULK_HEADER_SIZE);
 ...
 *
 * Claims USB interface 0 and launches init_ssm. The SSM calls
 * fpi_image_device_open_complete() when the hardware is ready
 * (or on error).
 */
static void
 ...
 * @dev: our #FpImageDevice
 *
 * Cancels any pending capture retry timer, then signals deactivation.
 * In-flight USB transfers submitted with fpi_device_get_cancellable() are
 * cancelled automatically by libfprint; capture_ssm_cb handles the resulting
 * G_IO_ERROR_CANCELLED gracefully.
 *
 * The retry timer must be explicitly removed here because g_timeout_add
 * sources are not affected by GCancellable. Failing to do so would leave
 * the timer callback calling fpi_ssm_jump_to_state on a freed SSM.
 */
static void
ft9201_deactivate (FpImageDevice *dev)
{
  ...
  if (self->capture_retry_source != 0)
    {
      g_source_remove (self->capture_retry_source);
      self->capture_retry_source = 0;
    }
